...
static u32 kbase_scale_dynamic_power(const u32 c, const u32 freq, const u32 voltage)
{
	...
	const u64 v_coeff = (14258541 * v2 - 9894430 * v3 - 5000208 * mv) / 1000000;

	const u64 f_coeff = (7231978 * f3 + 8333857 * f_MHz - 13653329 * f2) / 1000000;

	const u64 p_coeff =  v_coeff * f_coeff;

	return div_u64(p_coeff * ((c / 1000) + 1316), 1000000);
#endif
}
...
#else
	/* TODO: Wait for the sky1 static power model to be released*/
	return 0;
#endif
}
...
#else
	bool skip_utilization_scaling = true;
#endif
	int err = 0;
