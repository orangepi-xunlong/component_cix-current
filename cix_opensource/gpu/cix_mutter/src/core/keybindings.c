...
static const guint16 keycode_power = 124;
               ...
               ClutterKeyEvent      *event)
{
  ...
  /* power key only send release event. */
  if (event->type == CLUTTER_KEY_PRESS && keycode == keycode_power) {
    return FALSE;
  }

  /* we used to have release-based bindings but no longer. */
  if (event->type == CLUTTER_KEY_RELEASE && keycode != keycode_power) {
    return FALSE;
  }
