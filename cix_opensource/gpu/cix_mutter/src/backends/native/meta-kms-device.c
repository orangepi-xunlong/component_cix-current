  ...
  for (l = meta_kms_device_get_planes (device); l; l = l->next)
    {
      ...
      // When MST is connected, CIX's kernel will report more than one possible CRTC for a primary plane.
      // In this case, we need to check the CRTC ID first and choose the one it is associated with.
      uint64_t crtc_id = meta_kms_plane_get_prop_value(plane, META_KMS_PLANE_PROP_CRTC_ID);
      if (crtc_id == meta_kms_crtc_get_id(crtc))
      {
        return plane;
      }
      else if (meta_kms_plane_is_usable_with (plane, crtc))
      {
        return plane;
      }
    }
