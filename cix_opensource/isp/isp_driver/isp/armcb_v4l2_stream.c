				 ...
				 struct v4l2_format *f)
{
	...
	for (i = 0; i < tfmt->memplanes; i++) {
		...
		LOG(LOG_DEBUG, "[Stream#%d] plane %d: height:%d, bytesperline:%d, sizeimage:%d",
		    pstream->stream_id, i, f->fmt.pix_mp.height, f->fmt.pix_mp.plane_fmt[i].bytesperline,
		    f->fmt.pix_mp.plane_fmt[i].sizeimage);
